...
|---|---|---|---|
| 2026-05-20 | happyeat | **NEW (tiered delivery fee + wave dispatch + customer boost)**: (1) ค่าส่งคำนวณจากระยะ OSRM driving distance: 0–4km=35฿, >4–7km +6฿/km, >7–10km +7฿/km, >10km +8฿/km, ปัดขึ้นเป็นบาทเต็มด้วย `Math.ceil` (สูตรอยู่ใน `src/lib/delivery-fee.ts`); cart preview ค่าส่ง realtime ตอนเลือกที่อยู่. (2) เพิ่ม columns ใน `orders`: `delivery_distance_km`, `dispatch_wave smallint default 0`, `last_dispatched_at timestamptz default now()`, `delivery_fee_boost numeric default 0`, `awaiting_rider_boost boolean default false`. (3) Wave dispatch: wave 0 (สร้าง order) = ยิงให้ไรเดอร์ใกล้สุด 3 คนใน 4km; pg_cron `dispatch-tick-15s` เรียก `/api/public/hooks/dispatch-tick` ทุก 15 วินาที → wave 1 (4km ทุกคน) → wave 2 (6km) → wave 3 (8km) → wave 4 (หยุดยิง, เซ็ต `awaiting_rider_boost=true`). (4) RPC `boost_delivery_fee(_order_id, _amount)` (SECURITY DEFINER) — ลูกค้ากดเพิ่มค่าส่ง +10/+20/+30 ฿ ได้ตอน `awaiting_rider_boost=true` หรือ rider ยังว่าง → เพิ่ม `delivery_fee_boost`/`delivery_fee`/`total` พร้อม reset `dispatch_wave=3` ให้ cron ยิงรอบใหม่ทันที. (5) Trigger `enforce_orders_update_authorization` ขยายให้ลูกค้า boost ค่าส่งได้ (เพิ่ม `delivery_fee_boost`/`delivery_fee`/`total` พร้อมกันเท่าๆ กัน) นอกเหนือจาก field ที่ลูกค้าเคยแก้ได้. | **rider app: action ด่วน** — (1) Order ที่ rider เห็นจะมี `delivery_fee` รวม boost แล้ว ไม่ต้องคำนวณเพิ่ม แค่แสดงให้ครบ; (2) ถ้าจะโชว์ระยะทาง ใช้ `orders.delivery_distance_km` (ร้าน→ลูกค้า OSRM driving km); (3) **ห้าม** rider แตะ `delivery_fee_boost`/`dispatch_wave`/`awaiting_rider_boost` (trigger บล็อก); (4) ถ้าอยาก subscribe push wave ใหม่ — payload ตอนนี้ส่ง `data.orderId` + `data.url=/rider-dashboard` เหมือนเดิม คนละ wave จะมาเป็น push แยกแต่ละรอบ; (5) เน้น update `riders.current_lat/current_lng` สม่ำเสมอ — wave จะ filter ใน radius ถ้าไม่มี GPS = ตกรอบทุก wave |
...
| 2026-05-20 | happyeat | **NEW (native Android notification sounds)**: เพิ่มคอลัมน์ `profiles.notification_sound text default 'siren'` (`siren` \| `airhorn` \| `emergency`). FCM payload ทุก path (`sendOrderPush`, `sendStatusPush`, `pushToRiderTokens`) แนบ `android.notification.channel_id` + `android.notification.sound` ตาม pref ของผู้รับแต่ละคน. Channel IDs ต้องตรงกันทั้ง 2 แอป: `orders_siren`, `orders_airhorn`, `orders_emergency` (importance HIGH, vibration on). ไฟล์เสียง: `siren.mp3`, `airhorn.mp3`, `emergency.mp3` วางใน `android/app/src/main/res/raw/`. ฝั่ง happyeat สร้าง channel ผ่าน `PushNotifications.createChannel()` ตอน `_app.tsx` mount. | **rider app: action ด่วน** — (1) `bun add @capacitor/push-notifications` ถ้ายังไม่มี, (2) copy `src/lib/native-notifications.ts` จาก happyeat repo (มี channel IDs + creation logic), (3) เรียก `ensureOrderChannels()` ตอน app boot (root layout / dashboard mount), (4) เพิ่มหน้าตั้งค่าเสียง (เลือก 1 ใน 3) — save ลง `profiles.notification_sound`, (5) ขอไฟล์เสียง 3 ไฟล์จากห้องนี้ (มีให้ดาวน์โหลดที่ `/mnt/documents/sounds/`) วางใน `android/app/src/main/res/raw/` (ชื่อตัวพิมพ์เล็ก ไม่มี dash/space), (6) **rebuild APK** หลังวางไฟล์เสียง. หมายเหตุ: channel เปลี่ยนเสียงไม่ได้หลังสร้างแล้ว — ถ้าจะเปลี่ยนเสียงในอนาคตต้องใช้ channel ID ใหม่ (เช่น `orders_siren_v2`) ทั้ง 2 ฝั่งพร้อมกัน |
